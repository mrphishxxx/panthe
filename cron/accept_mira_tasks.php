...
while ($birj = $accs->FetchRow()) {
    ...
    if (count($logins) == 0) {
        $driver->quit();
        echo "Ошибка отправления, поле 'input[@name='UserLogin']' - не найдено";
        continue;
    }
    ...
    if (count($pass) == 0) {
        $driver->quit();
        echo "Ошибка отправления, поле 'input[@name='UserPassword']' - не найдено";
        continue;
    }
    ...
    if (count($checkall) === 0) {
        $driver->quit();
        echo "WARNING. Нет checkbox выбрать все(может нет статей для принятия)";
        exit();
    }
