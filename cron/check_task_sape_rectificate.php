...
$tasks = array();
$model = $db->Execute("SELECT * FROM zadaniya_new z WHERE z.rectificate='0' AND z.vrabote='0' AND z.vipolneno='0' AND z.dorabotka='0' AND z.navyklad='0' AND z.vilojeno='0'");
...
if (!empty($id_user_sape) && !empty($tasks)) {
    foreach ($tasks as $id => $sape_id) {
        ...
        if (isset($task["status"]) && $task["status"] == 0) {
            $body .= $id."(".$sape_id.");\r\n";
            $db->Execute("UPDATE zadaniya_new SET vrabote = '0', vilojeno='0', vipolneno='0', dorabotka = '0', navyklad = '0', rectificate = '1' WHERE id=" . $id);
        }
...
$message["html"] = $body;
$message["text"] = "";
$message["subject"] = "Проверка отмененных заданий из Сапы";
