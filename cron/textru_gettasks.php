...
$users = array();
$body = "";
$copywriters = $db->Execute('SELECT email FROM admins WHERE type = "copywriter" AND active = "1"')->GetAll();
...
if (!empty($check_tasks)) {
    foreach ($check_tasks as $task) {
        ...
        if (!empty($error_for_copywriter)) {
            ...
            $message["to"][0] = $users[$task["copywriter"]]["email"];
            try {
            ...
            $db->Execute('UPDATE zadaniya_new SET rework = "1", navyklad="0" WHERE id = ' . $task["id"]);
        }
