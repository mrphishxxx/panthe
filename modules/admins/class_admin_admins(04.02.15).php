...
class admins {
    ...
    function etxt($db) {
        ...
        $sid = (int) $_GET['sid'];
        $pass = 'M2Lp2pG2';
        $db_res = $db->Execute("SELECT * FROM `zadaniya` WHERE sid='" . $sid . "' AND uid='" . $uid . "' AND etxt=0");
        ...
        while ($db_res2 = $db_res->FetchRow()) {
            ...
            if (($uid != 20) && ($uid != 55)) {
                ...
                if ($db_res2['lay_out'] == 1) {
                    $cur_balans -= 15;
                } else if ($db_res2['sistema'] == "http://miralinks.ru/" || $db_res2['sistema'] == "http://pr.sape.ru/" || $db_res2['sistema'] == "http://getgoodlinks.ru/") {
                    $colvos = 2000;
                    switch ($sinfo['cena']) {
                    ...
                    //$cur_balans -= (2 * $cena) + (round($cena / 10)) + 13;
                } else {
    ...
    function load($db) {
        ...
        $query = $db->Execute("select * from site_b where uid=$uid");
        $res = $query->FetchRow();
    ...
    function view($db) {
        ...
        while ($res = $query->FetchRow()) {
            ...
            $admins = str_replace('[id]', $res['id'], $admins);
            $uid = $res['id'];
            $z1 = $db->Execute("select count(*) from zadaniya where vrabote=1 and (vipolneno=0 or vipolneno IS NULL) and (dorabotka=0 or dorabotka IS NULL) and (navyklad=0 or navyklad IS NULL) and (vilojeno=0 or vilojeno IS NULL) and uid=$uid")->FetchRow();
            $z2 = $db->Execute("select count(*) from sayty where uid=$uid")->FetchRow();
            $admins = str_replace('[z1]', $z1['count(*)'], $admins);
            $admins = str_replace('[z2]', $z2['count(*)'], $admins);
            ...
            if (!@$_SESSION["admins"]["id"]) {
                $admins = str_replace('[rights]', "style='display:none'", $admins);
            } else {
                $admins = str_replace('[rights]', "", $admins);
            }
        }
    ...
    function sayty_add($db) {
        ...
        if (!$send) {
            ...
            $db->Execute("insert into sayty(uid, url, url_admin, login, pass, price, gid, getgoodlinks_id, sape_id, miralinks_id, site_subject, site_subject_more, cms, obzor_flag, news_flag, subj_flag, bad_flag, anons_size, pic_width, pic_height, pic_position, site_comments) values 
					($uid, '$url', '$url_admin', '$login', '$pass', '$price', '$gid', '$getgoodlinksId', '$sape_id', '$miralinks_id', '$site_subject', '$site_subject_more', '$cms', '$obzor_flag', '$news_flag', '$subj_flag', '$bad_flag', '$anons_size', '$pic_width', '$pic_height', '$pic_position', '$site_comments')");
    ...
    function sayty_edit($db) {
        ...
        if (!$send) {
            ...
            $db->Execute("update sayty set uid='$owner_id', colvos='$colvos', login='$login', gid='$gid', getgoodlinks_id='$getgoodlinksId', sape_id='$sape_id', miralinks_id='$miralinks_id', pass='$pass', url='$url', url_admin='$url_admin', price='$price_iforget', cena='$price_etxt', moder_id='$moder_id', comment_viklad='$comment_viklad', question_viklad='$question_viklad',
    ...
    function zadaniya_edit($db) {
        ...
        if (!$send) {
            ...
            $sign = md5(implode('', $data) . md5($pass . 'api-pass'));
            $url = 'https://www.etxt.ru/api/json/?' . implode('&', $data2) . '&sign=' . $sign;
            $profil .= microtime() . "  - BEFORE 1 curl - getResults" . "\r\n";
            if ($curl = curl_init()) {
                curl_setopt($curl, CURLOPT_URL, $url);
                curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
            ...
            $profil .= microtime() . "  - AFTER 1 curl - getResults" . "\r\n";
            ...
            $profil .= microtime() . "  - BEFORE 2 curl - listTasks" . "\r\n";
            ...
            $profil .= microtime() . "  - AFTER 2 curl - listTasks" . "\r\n";
            ...
            if ($lay_out) {
                $content = str_replace('[lay_out_text]', "checked", $content);
            }
            ...
            if ($sinfo["subj_flag"] == 1) {
                $content = str_replace('[themes]', "<span class='small red'>Берутся только тематичные задания (" . $sinfo["site_subject"] . (!empty($sinfo["site_subject_more"]) ? '(' . $sinfo["site_subject_more"] . ')' : '') . ")</span>", $content);
            } else {
                $content = str_replace('[themes]', "<span class='small red'>Берутся все задания.</span>", $content);
            }
            ...
            if (isset($_REQUEST['morework']) && ($_REQUEST['morework'] == 1) && $task_id) {
                ...
                curl_exec($curl);
                curl_close($curl);
                $profil .= microtime() . "  - AFTER 3 curl - cancelTask" . "\r\n";
            } else if (isset($_REQUEST['morework']) && ($_REQUEST['morework'] == 0) && $task_id) {
                $pass = 'M2Lp2pG2';
                $query_sign = "method=tasks.paidTasktoken=29aa0eec2c77dd6d06e23b3faaef9eed";
                $sign = md5($query_sign . md5($pass . 'api-pass'));
                $profil .= microtime() . "  - BEFORE 3 curl - paidTask" . "\r\n";
                ...
                curl_exec($curl);
                curl_close($curl);
                $profil .= microtime() . "  - AFTER 3 curl - paidTask" . "\r\n";
            }
    ...
    function s_mail($res, $step) {

        $content = file_get_contents(PATH . 'modules/admins/tmp/mess/naviklad.tpl');
        $content = str_replace('[url_s]', $res[''], $content);
        $content = str_replace('[url_ks]', $res[''], $content);
        /* тема/subject */
		...
		<p>Поздравляем! Вы успешно зарегистрировались в статусе ' . $stat . '. Ваша работа будет осуществляться через личный кабинет пользователя. Для входа в личный кабинет используйте логин и пароль указанными при регистрации.
		</p>
        ...
        $message["from_name"] = "iforget";
        $message["to"] = array();
        $message["to"][0] = array("email" => $data['login'], "name" => $data['fio']);
        $message["track_opens"] = null;
        $message["track_clicks"] = null;
        $message["auto_text"] = null;
    ...
    function ticket_answer($db) {
		...
		';
            $subject = "[Сообщение в тикете от админимстрации IFORGET]";
            ...
            $message["from_name"] = "iforget";
            $message["to"] = array();
            $message["to"][0] = array("email" => $client_mail);
            $message["track_opens"] = null;
            $message["track_clicks"] = null;
            $message["auto_text"] = null;
    ...
    function allsites($db) {
        ...
        $active_uid = "(" . implode(",", $active_uid) . ")";
        $profil .= microtime() . "  - QUERY 1 - BEFORE GET TASKS" . "\r\n";
        $sayty = '';
        $query = $db->Execute("
            SELECT 
                s.*, 
                count(IF (z.navyklad = 1, 1, NULL)) as navyklad,
                count(IF (z.vrabote = 1 and (z.dorabotka=0 or z.dorabotka IS NULL) and (z.vipolneno=0 or z.vipolneno IS NULL) and (z.navyklad=0 or z.navyklad IS NULL) and (z.vilojeno=0 or z.vilojeno IS NULL), 1, NULL)) as vrabote,
                count(IF (z.vilojeno = 1, 1, NULL)) as vilojeno,
                count(IF (z.dorabotka = 1, 1, NULL)) as dorabotka,
                count(IF((s.id = z.sid) and (z.vrabote=0 or z.vrabote IS NULL) and (z.dorabotka=0 or z.dorabotka IS NULL) and (z.vipolneno=0 or z.vipolneno IS NULL) and (z.navyklad=0 or z.navyklad IS NULL) and (z.vilojeno=0 or z.vilojeno IS NULL), 1, NULL)) as neobrabot 
            FROM sayty s 
            LEFT JOIN zadaniya z ON z.sid = s.id
            WHERE s.uid IN $active_uid GROUP BY s.id ORDER BY navyklad DESC, neobrabot DESC
        ");
        $profil .= microtime() . "  - QUERY 1 - AFTER GET TASKS" . "\r\n";
        $n = 1;
        while ($res = $query->FetchRow()) {
            $cur_balans = $this->getUserBalans($res['uid'], $db, 0);
            $profil .= microtime() . "  - getUserBalans" . $res['uid'] . "\r\n";

            if (($cur_balans < 45) && $res['uid'] != 20 && ($res['navyklad'] == 0 && $res['vrabote'] == 0 && $res['vilojeno'] == 0 && $res['dorabotka'] == 0 && $res['neobrabot'] == 0)) {
                continue;
            }

            $birjs = $db->Execute("select * from birjs where uid=" . $res['uid'])->FetchRow();
            ...
            $sayty = str_replace('[balans]', $cur_balans, $sayty);
            $sayty = str_replace('[url]', $res['url'], $sayty);
            $sayty = str_replace('[id]', $res['id'], $sayty);
            $sayty = str_replace('[uid]', $res['uid'], $sayty);
            ...
            $sayty = str_replace('[z1]', $res['vrabote'], $sayty);
            $sayty = str_replace('[z2]', $res['dorabotka'], $sayty);
            $sayty = str_replace('[z4]', $res['neobrabot'], $sayty);
            $sayty = str_replace('[z5]', $res['vilojeno'], $sayty);
            $sayty = str_replace('[z7]', $res['navyklad'], $sayty);
            ...
            $birjs_info = $db->Execute("SELECT * FROM birjs WHERE uid=" . $res['uid']);
            $birjs_count = $db->Execute("SELECT count(*) as count FROM birjs WHERE uid=" . $res['uid']);
            $birjs_count = $birjs_count->FetchRow();
            $profil .= microtime() . "  - QUERY 2 && 3  - GET birjs" . "\r\n";
            $i = 1;
            while ($birjs = $birjs_info->FetchRow()) {
                ...
                $birgi_info = $db->Execute("SELECT * FROM birgi WHERE id=" . $birjs['birj']);
                $birgi_info = $birgi_info->FetchRow();

                $birj = str_replace('[birja]', $birgi_info['url'], $birj);
                ...
                $edit_comment = '<td class="edit row_tt" rowspan="' . $birjs_count["count"] . '">
                                    <a href="?module=admins&action=birj&action2=edit_comment&id=' . $res['uid'] . '" class="ico"></a>
                                </td>';
                if ($i == 1) {
                    $user = $db->Execute("select comment_viklad from admins where id=" . $res['uid']);
                    $user = $user->FetchRow();
    ...
    function createAdminTicket($db) {
        ...
        if (@$_REQUEST['action2'] == "add") {
            ...
            $body = "Добрый день!<br/><br/>
			Вам поступил новый тикет. Для просмотра <a href='http://iforget.ru/user.php?action=ticket&action2=view&tid=" . $lastId . "'>перейдите данной ссылке</a>.
			";
            $body .= "<br /><br /> Оставить и почитать отзывы Вы сможете в нашей ветке на <a href='http://searchengines.guru/showthread.php?p=12378271'>серчах</a><br/><br/>С уважением,<br/>Администрация проекта iForget.";
    ...
    function zadaniya_dubl($db) {
        ...
        if ($navyklad == 1) {
            ...
            $message["from_name"] = "iforget";
            $message["to"] = array();
            $message["to"][0] = array("email" => $res['email']);
            $message["track_opens"] = null;
            $message["track_clicks"] = null;
            $message["auto_text"] = null;
    ...
    function change_status_task($db) {
        ...
        if (!empty($uid) && !empty($task) && !empty($status) && $status == "dorabotka") {
            ...
            if ($task['copywriter'] != 0) {
                ...
                $mail["to"][1] = array("email" => "abashevav@gmail.com");
    ...
    function statistics_money($db) {
        ...
        if (!empty($date)) {
            $tasks = $db->Execute("SELECT * FROM completed_tasks WHERE date >= '$date' ORDER BY date ASC"); // AND date < '2014-10-28 00:00:00'
            $sapes = $db->Execute("SELECT * FROM zadaniya_new WHERE (etxt = 1 OR copywriter != 0) AND date >= '$time' ORDER BY date ASC");
        } else {
            $tasks = $db->Execute("SELECT * FROM completed_tasks ORDER BY date ASC");
        ...
        while ($task = $tasks->FetchRow()) {
            ...
            if (!isset($stat[$date_time_array[$filter]])) {
                $stat[$date_time_array[$filter]] = array("15" => 0, "45" => 0, "61" => 0, "93" => 0, "60" => 0, "76" => 0, "111" => 0, "0" => 0, "40" => 0, "90" => 0, "100" => 0, "4" => 0);
            }
        ...
        foreach ($money as $key => $value) {
            ...
            $tr .= "<td>" . (round(($stat[$key][93] + $stat[$key][61] + $stat[$key][45] + $stat[$key][15] + $stat[$key][60] + $stat[$key][76] + $stat[$key][111]))) . "</td>";
            $tr .= "<td>" . $value . "</td>";
            $tr .= "</tr>";
            $table .= $tr;
            $sum += $value;
            $count += ($stat[$key][93] + $stat[$key][61] + $stat[$key][45] + $stat[$key][15] + $stat[$key][60] + $stat[$key][76] + $stat[$key][111]);
        }
        ...
        while ($task = $sapes->FetchRow()) {
            ...
            $stat[$date_time_array[$filter]][round($price)] += 1;
            $money[$date_time_array[$filter]] += $price;
    ...
    function articles($db) {
        ...
        if (!empty($tasks)) {
            foreach ($tasks as $value) {
                ...
                } elseif ((empty($value["task_id"]) && empty($value["copywriter"])) && $value["nof_chars"] == 2000) {
    ...
    function article_edit($db) {
        ...
        if (!$send) {
            ...
            $profil .= microtime() . "  - BEFORE 1 curl" . "\r\n";
            $uniq = 0;
            $etxt_action = "";
            if (!empty($task['task_id']) && $check_result == true && !$task['overwrite'] && $task['vipolneno'] = 0) {
                $pass = 'M2Lp2pG2';
                $params = array('method' => 'tasks.getResults', 'token' => '29aa0eec2c77dd6d06e23b3faaef9eed', 'id' => $task['task_id']);
                ksort($params);
                $data = array();
                $data2 = array();
                foreach ($params as $k => $v) {
                ...
                if ($file_path && empty($task['text'])) {
                    $text = @file_get_contents($file_path);
                    $cur_text = iconv('cp1251', 'utf-8', $text);
                    $profil .= microtime() . "  - AFTER file_get_contents FOR text file" . "\r\n";
                    if (!$task['overwrite'])
                        $content = str_replace('[text]', $cur_text, $content);
                }
            }
            $profil .= microtime() . "  - BEFORE 2 curl - login in sape" . "\r\n";
            $cookie_jar = tempnam(PATH . 'temp', "cookie");
            ...
            $content = str_replace('[uniq]', ($task['uniq'] != 0) ? $task['uniq'] : $uniq, $content);
