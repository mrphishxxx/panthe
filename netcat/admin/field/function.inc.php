...
function ColumnInMessage($FieldID, $type, $db, $NewFieldName = false) {
    ...
    switch ($type) {
        case 1: $oper = "ADD";
            break;
        case 2: $oper = $NewFieldName ? "CHANGE" : "MODIFY";
            break;
        case 3: $oper = "DROP";
            break;
    }
...
function FieldForm($FieldID, $Id, $isSys = 0, $action = "index.php", $FormName = '', $FormID = '', $Additional = '', $isWidget = 0) {
  ...
  <td>" . CONTROL_FIELD__EDITOR_EMBED_TO_FIELD . "</td>";
    ...
    for ($i = 0; $i <= 2; $i++) {
        $html .= "<td bgcolor='" . ( $format_text['fck'] == $i ? "#E7E7E7" : "#FFFFFF") . "'>" .
                nc_admin_radio_simple('format_fck', $i, '', (isset($Array['format_fck']) && $Array['format_fck'] == $i) || $format_text['fck'] == $i, '', "class='w'") . "
      </td>";
    }
  ...
  <br/>" . nc_admin_checkbox_simple('format_usereditor', 1, CONTROL_FIELD_USER_EDITOR, isset($Array['format_usereditor']) ?  $Array['format_usereditor'] : $format_text['usereditor']) . "
  <br/><br/>
  </div>";
...
function FieldCompleted() {
    ...
    global $format_height, $format_width, $format_html, $format_br, $format_fck, $format_bbcode, $format_usereditor, $use_calendar, $protect_email;
    ...
    // Update field
        ...
        if (!$isSys && !$widgetclass_id) {
            ...
            if (strtolower($FieldName) != strtolower($OldFieldName)) {
