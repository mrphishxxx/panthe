...
<?php $wysiwyg_dir = $nc_core->ROOT_FOLDER . "editors/"; ?>
<?php if ( is_dir($wysiwyg_dir . 'FCKeditor') ): ?>
    <tr>
...
<?php endif; ?>
                <tr>
                    <td width='70%'><?= NETCAT_SETTINGS_INLINE_EDIT_CONFIRMATION ?>:</td>
                    <td><?= $Array['InlineEditConfirmation'] == 1 ? NETCAT_SETTINGS_INLINE_EDIT_CONFIRMATION_ON : NETCAT_SETTINGS_INLINE_EDIT_CONFIRMATION_OFF ?></td>
                </tr>
    <tr>
...
<?php $wysiwyg_dir = $nc_core->ROOT_FOLDER . "editors/"; ?>
                        <select name='EditorType' style='margin-left: 10px;'>
                            <?php if ( is_dir($wysiwyg_dir . 'FCKeditor') ): ?><option value='<?= NC_FCKEDITOR ?>' <?= ( $Array['EditorType'] == NC_FCKEDITOR ? "selected" : "") ?>><?= NETCAT_SETTINGS_EDITOR_FCKEDITOR ?></option><?php endif; ?>
                            <?php if ( is_dir($wysiwyg_dir . 'ckeditor') ): ?><option value='<?= NC_CKEDITOR ?>' <?= ( $Array['EditorType'] == NC_CKEDITOR ? "selected" : "") ?>><?= NETCAT_SETTINGS_EDITOR_CKEDITOR ?></option><?php endif; ?>
                            <?php if ( is_dir($wysiwyg_dir . 'tinymce') ): ?><option value='<?= NC_TINYMCE ?>' <?= ( $Array['EditorType'] == NC_TINYMCE ? "selected" : "") ?>><?= NETCAT_SETTINGS_EDITOR_TINYMCE ?></option><?php endif; ?>
                        </select>
                    </td>
                </tr>
                <?php /*
    ...
    <?= NETCAT_SETTINGS_EDITOR_SKINS ?>:
                ...
                */ ?>
                <tr>
                    <td colspan="2">
                        <?= nc_admin_checkbox_simple('InlineEditConfirmation', 1, "" . NETCAT_SETTINGS_INLINE_EDIT_CONFIRMATION . "", $Array['InlineEditConfirmation']) ?>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
...
function SettingsCompleted() {
    ...
    $p = array('ProjectName', 'UserEmailField', 'SpamFromName', 'SpamFromEmail', 'EditorType', 'EmbedEditor', 'InlineEditConfirmation',
