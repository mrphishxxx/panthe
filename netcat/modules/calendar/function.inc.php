        ...
        if ($daysMask[$i - 1]) {
            ...
            if ($popup) {
                ...
                if ( isset($mirror_data[ $daysMask[$i - 1] ]) && !empty($mirror_data[ $daysMask[$i - 1] ]) ) {
					...
					$_hidden_url = $mirror_data[ $daysMask[$i - 1] ]['Hidden_URL'];
				}
				else {
					$_cc_id = $daysMask[$i - 1];
					$_sub_id = $cc_data[ $daysMask[$i - 1] ]['Subdivision_ID'];
					$_hidden_url = $cc_data[ $daysMask[$i - 1] ]['Hidden_URL'];
				}

                if ($nc_core->admin_mode) {
                    $Hidden_URL = $nc_core->SUB_FOLDER.$nc_core->HTTP_ROOT_PATH.'?sub='.$_sub_id.'&amp;cc=' . $_cc_id;
