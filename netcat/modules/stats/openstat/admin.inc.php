...
function write_template($ParentTemplateID, &$NoCounterTemplates, $count = 0) {
    ...
    $Result = $db->get_results("SELECT Template_ID,Description,File_Mode FROM `Template` where `Parent_Template_ID`='".$ParentTemplateID."' AND `Template_ID`<>'".$nc_core->get_settings('EditDesignTemplateID')."' ORDER BY Template_ID", ARRAY_N);
    if ($Result) {
