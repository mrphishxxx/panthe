...
/* $Id: nc_component.class.php 8618 2013-01-16 08:50:11Z vadim $ */
...
class nc_Component extends nc_Essence {
    ...
    public function get_for_cc($sub_class_id, $class_id, $nc_ctpl, $nc_tpl_in_cc = 0) {
        $nc_core = nc_Core::get_object();
        ...
        // выбор по шаблону nc_ctpl переданому в s_list
        ...
        // выбор по номеру компонента если никакие шаблоны не подолши
        foreach ($this->data as $id => $v) {
 ...
 public function get_search_query($srchPat) {
	...
	/**
	 * Sync MessageXX with autoincrement into the Class table
	 */
	public function sync_mc () {
		// get MessageXX tables
		$message_tables = $db->get_results("SHOW TABLES LIKE 'Message%'", ARRAY_A);
		// autoincrement
		$class_autoincrement = 0;
		// find max id
		foreach ($message_tables as $row) {
			foreach ($row as $value) {
				preg_match("/^Message(\d+)$/is", trim($value), $matches);
				if (isset($matches[1]) && is_numeric($matches[1]) && $matches[1] > $class_autoincrement) {
					$class_autoincrement = $matches[1];
				}
			}
		}
		// set autoincrement
		...
		$db->query("ALTER TABLE `Class` AUTO_INCREMENT = " . $class_autoincrement);	
	}
}
?>
